...
namespace _211229001_Anti_SRP
{
    ...
    public class Customer_Anti_SRP_01
    {
        ...
        public Customer_Anti_SRP_01(int userno, string userName, string userSurname, string adress, double UserAmount, int point, CustomerType customerType, string email, string password, bool loginStatus)
        {
            this.UserNo = userno;
        ...
        public void AddBasket(int USerCode, int ProductCode, List<Products_Anti_SRP_01> ProductsList)
        {
            bool product = false;
        ...
        public void ShowBasket()
        {
            ...
            Product.basket.ForEach(delegate (Products_Anti_SRP_01 products)
            {
        ...
        public void Buy(int UserCode, List<Products_Anti_SRP_01> ProductsList)
        {
            bool user = false;
            double total = 0;
            CustomerList.ForEach(delegate (Customer_Anti_SRP_01 customer)
            {
                if (customer.UserNo == UserCode)
                {
                    ...
                    if (customer.LoginStatus == true)
                    {
                        if (Product.basket.Count > 0)
                        {
                            Product.basket.ForEach(delegate (Products_Anti_SRP_01 product)
                            {
                                total += product.Price;
                            });
                        }
        ...
        public void ShowProductsCategory(CategoryType category, List<Products_Anti_SRP_01> ProductsList)
        {
            bool Category = false;
            ProductsList.ForEach(delegate (Products_Anti_SRP_01 products)
            {
