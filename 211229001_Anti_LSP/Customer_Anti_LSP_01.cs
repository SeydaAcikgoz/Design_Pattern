...
namespace _211229001_Anti_LSP
{
    ...
    public class Customer_Anti_LSP_01
    {
        ...
        public void AddBasket(int USerCode, int ProductCode, List<Products_Anti_LSP_01> ProductsList)
        {
            bool product = false;
        ...
        public void ShowBasket()
        {
            ...
            Product.basket.ForEach(delegate (Products_Anti_LSP_01 products)
            {
        ...
        public void Shipping(Customer_Anti_LSP_01 customer)
        {
            Product.basket.ForEach(delegate (Products_Anti_LSP_01 products)
            {
                Console.WriteLine(products.Code + "\t" + products.ProductName + "\t" + products.categoryType + "\t" + products.Price + "\t" + products.Stock + "\t");
        ...
        public void Buy(int UserCode, List<Products_Anti_LSP_01> ProductsList)
        {
            bool user = false;
            double total = 0;
            ...
            CustomerList.ForEach(delegate (Customer_Anti_LSP_01 customer)
            {
                if (customer.UserNo == UserCode)
                {
                    ...
                    if (customer.LoginStatus == true)
                    {
                        if (Product.basket.Count > 0)
                        {
                            Product.basket.ForEach(delegate (Products_Anti_LSP_01 product)
                            {
                                total += product.Price;
                            });
                        }
        ...
        public void ShowProductsCategory(CategoryType category, List<Products_Anti_LSP_01> ProductsList)
        {
            bool Category = false;
            ProductsList.ForEach(delegate (Products_Anti_LSP_01 products)
            {
