...
{
    public class Customer_SRP_01 : ITransactions_SRP_01
    {
        public int UserNo;
        ...
        public List<Customer_SRP_01> CustomerList = new List<Customer_SRP_01>();


        public bool login = false;
        public Customer_SRP_01()
        {
        }


        public Customer_SRP_01(int userno, string userName, string userSurname, string adress, double UserAmount, int point, string customerType, string email, string password, bool loginStatus)
        {
            this.UserNo = userno;
        ...
        Products_SRP_01 Product = new Products_SRP_01();
        ...
        public void ShowBasket()
        {
            ...
            Product.basket.ForEach(delegate (Products_SRP_01 products)
            {
        ...
        public void IncreasePoints(Customer_SRP_01 customer)
        {
            customer.Point += Product.basket.Count() * 10;
            Console.WriteLine("Current point:" + customer.Point);
        }
        public void Payment(Customer_SRP_01 customer, double total)
        {
            ...
            CustomerList.ForEach(delegate (Customer_SRP_01 customer)
            {
                if (customer.UserNo == UserCode)
                {
                    ...
                    if (customer.LoginStatus == true)
                    {
                        if (Product.basket.Count > 0)
                        {
                            Product.basket.ForEach(delegate (Products_SRP_01 product)
                            {
                                total += product.Price;
                            });
                        }
