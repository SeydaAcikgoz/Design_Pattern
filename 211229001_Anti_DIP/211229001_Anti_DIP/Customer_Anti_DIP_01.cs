...
{
    public class Customer_Anti_DIP_01 : ITransactions_Anti_DIP_01
    {
        public int UserNo;
        ...
        public List<Customer_Anti_DIP_01> CustomerList = new List<Customer_Anti_DIP_01>();


        public bool login = false;
        public Customer_Anti_DIP_01()
        {
        }


        public Customer_Anti_DIP_01(int userno, string userName, string userSurname, string adress, double UserAmount, int point, string customerType, string email, string password, bool loginStatus)
        {
            this.UserNo = userno;
        ...
        public void ShowBasket()
        {
            ...
            Product.basket.ForEach(delegate (Products_Anti_DIP_01 products)
            {
        ...
        public void IncreasePoints(Customer_Anti_DIP_01 customer)
        {
            customer.Point += Product.basket.Count() * 10;
            Console.WriteLine("Current point:" + customer.Point);
        }
        public void Payment(Customer_Anti_DIP_01 customer, double total)
        {
            Console.WriteLine("Price:" + total);

            if (customer.customerType.Equals("Gold"))
            {
                GoldCustomer_Anti_DIP_01 gold=new GoldCustomer_Anti_DIP_01();
                total = total - (total * gold.discount / 100);
            }
            else if (customer.customerType.Equals("Silver"))
            {
                SilverCustomer_Anti_DIP_01 silver = new SilverCustomer_Anti_DIP_01();
                total = total - (total * silver.discount / 100);
            }
            else if (customer.customerType.Equals("Bronz"))
            {
                BronzCustomer_Anti_DIP_01 bronz=new BronzCustomer_Anti_DIP_01();
                total = total - (total * bronz.discount / 100);
            }
            ...
            CustomerList.ForEach(delegate (Customer_Anti_DIP_01 customer)
            {
                if (customer.UserNo == UserCode)
                {
                    ...
                    if (customer.LoginStatus == true)
                    {
                        if (Product.basket.Count > 0)
                        {
                            Product.basket.ForEach(delegate (Products_Anti_DIP_01 product)
                            {
                                total += product.Price;
                            });
                        }
