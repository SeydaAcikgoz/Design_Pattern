...
{
    public class Customer_DIP_01 : IProductsTransactions_DIP_01, ICustomerTransactions_DIP_01
    {
        public int UserNo;
        ...
        public void ShowBasket()
        {
            ...
            Product.basket.ForEach(delegate (Products_DIP_01 products)
            {
        ...
        public void Payment(Customer_DIP_01 customer, double total)
        {
            ...
            {
                GoldCustomer_DIP_01 gold = new GoldCustomer_DIP_01();
                total = gold.CalculatePrice(total);
            }
            else if (customer.customerType.Equals("Silver"))
            {
                SilverCustomer_DIP_01 silver = new SilverCustomer_DIP_01();
                total = silver.CalculatePrice(total);
            }
            ...
            CustomerList.ForEach(delegate (Customer_DIP_01 customer)
            {
                if (customer.UserNo == UserCode)
                {
                    ...
                    if (customer.LoginStatus == true)
                    {
                        if (Product.basket.Count > 0)
                        {
                            Product.basket.ForEach(delegate (Products_DIP_01 product)
                            {
                                total += product.Price;
                            });
                        }
